...
<head>
	...
	var res = false;
		...
		function verifyAccount(asyncFlag){
			...
			//1.获取账号
			var $account = $("#account");
			if($account.val() != ""){//去空，自己做
				//2.根据账号到数据库中查询并返回结果，提示用户账号是否可用
				$.ajax({
					...
					success:function(msg){
						if("true" != msg){//说明账号以及存在
							alert("该账号已经被使用，请使用其他账号！");
							$account.focus();
							res = false;
						} else{
							res = true;
...
<form id="form" name="form" action="${basePath }nsfw/user_add.action" method="post" enctype="multipart/form-data">
    ...
    <table id="baseInfo" width="100%" align="center" class="list" border="0" cellpadding="0" cellspacing="0"  >
        ...
        <tr>
            ...
            <td>
            	<s:checkboxlist name="roleIds" list="#roleList" listKey="roleId" listValue="name">
            	</s:checkboxlist>
            </td>
        </tr>
        <tr>
